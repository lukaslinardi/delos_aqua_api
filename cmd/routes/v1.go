...
func getV1(router, routerJWT *mux.Router, handler api.Handler) {
	...
	router.HandleFunc("/v1/update-farm", handler.Farm.Farm.GetFarm).Methods(http.MethodPut)
	...
	router.HandleFunc("/v1/pond", handler.Pond.Pond.GetPond).Methods(http.MethodGet)
